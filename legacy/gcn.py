...
class FCN(nn.Module):
    ...
    def forward(self, x, additional=False):
        ...
        fs1 = self.refine6(F.upsample(gcfm1, fm3.size()[2:], mode='bilinear') + gcfm2)
        fs2 = self.refine7(F.upsample(fs1, fm2.size()[2:], mode='bilinear') + gcfm3)
        fs3 = self.refine8(F.upsample(fs2, pool_x.size()[2:], mode='bilinear') + gcfm4)
        fs4 = self.refine9(F.upsample(fs3, conv_x.size()[2:], mode='bilinear') + gcfm5)
        out = self.refine10(F.upsample(fs4, input_size[2:], mode='bilinear'))
